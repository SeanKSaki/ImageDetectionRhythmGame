...
label rhythm_game_entry_label:
    ...
    while isinstance(selected_song, Song):
        ...
        $ renpy.notify('Match the handsigns that are on the tracks by the time the notes reach the ends of the tracks to score points. Good Luck!')
...
screen rhythm_game(rhythm_game_displayable):
    ...
    key 'K_f' action NullAction()
    key 'K_j' action NullAction()
    key 'K_k' action NullAction()
    ...
    vbox:
        ...
        showif rhythm_game_displayable.has_game_started:
            text 'Score: ' + str(rhythm_game_displayable.score):
                ...
                size 30
        showif rhythm_game_displayable.has_game_started:
            text 'Combo: ' + str(rhythm_game_displayable.combo):
                color '#fff'
                size 30
...
init python:
    ...
    class Song():
        def __init__(self, name, audio_path, beatmap_path, beatmap_stride=3): # make a difficulty slider at some point && and scoring with combo
    ...
    class RhythmGameDisplayable(renpy.Displayable):
        ...
        def __init__(self, song):
            ...
            self.note_spacing = (self.track_bar_height - self.note_width) / 2
            self.note_spacing_large = (self.track_bar_height - self.note_width * self.zoom_scale) / 2
            # place the hit text some spacing from the end of the track bar
            self.hit_text_yoffset = 30

            # since the notes are scrolling from the screen left to right
            ...
            # drawing position
            self.track_bar_spacing = (config.screen_height - self.spacing * 2) / (self.num_track_bars - 1)
            # the spacing of each track bar
            self.track_spacings = {
            track_idx: self.spacing + track_idx * self.track_bar_spacing
            ...
            self.miss_threshold = 0.1 # seconds
        ...
        def render(self, width, height, st, at):
            ...
            for track_idx in range(self.num_track_bars):
                ...
                spacing = self.track_spacings[track_idx]
                # y = 0 starts from the top
                render.place(self.track_bar_drawable, x = 0, y=spacing)
                    ...
                    for onset, note_timestamp in self.active_notes_per_track[track_idx]:
                        ...
                        if self.onset_hits[onset] is None:
                            ...
                            if abs(curr_time - onset) <= self.hit_threshold:
                                ...
                                note_spacing = spacing + self.note_spacing_large
                                if abs(curr_time - onset) <= self.miss_threshold:
                                    self.onset_hits[onset] = 'miss'
                                    #render.place(self.miss_text_drawable, x=self.track_bar_width, y=spacing + self.hit_text_yoffset)
                            else:
                                note_drawable = self.note_drawables[track_idx]
                                note_spacing = spacing + self.note_spacing
                            ...
                            x_offset = self.track_bar_width - note_distance_from_top
                            render.place(note_drawable, x=x_offset, y=note_spacing)
                        ...
                        if self.onset_hits[onset] == 'miss':
                            ...
                            render.place(self.good_text_drawable, x=self.track_bar_width, y=spacing + self.hit_text_yoffset)
                        elif self.onset_hits[onset] == 'perfect':
                            render.place(self.perfect_text_drawable, x=self.track_bar_width, y=spacing + self.hit_text_yoffset)
